...
class Cell:
	def __init__(self,cellID,locn,synvars,celltype,morphFileName,modeltype='Multi'):
		...
		if modeltype == 'Multi':
			...
			# Prevent "double-booking" of segments into layers
			self.celltype.separateSeg(self)
			...
			for dendType in self.layerDict:
				for synType in self.synGroups:
					self.addSegmentSynapse(self.layerDict[dendType],self.dist2sec[dendType],self.distKeys[dendType],self.synGroups[synType])
			...
			if 'granule' in celltype:
				self.gcl1 = h.Section()
				self.gcl2 = h.Section()
				self.gcl1.connect(self.soma,1,0)
				self.gcl2.connect(self.soma,1,0)
			
			if 'ca3' in celltype:
				self.axon = h.Section()
				self.axon.connect(self.soma,1,0)
				
				self.basal = h.Section()
				self.basal.connect(self.soma,1,0)
				
				self.apic = h.Section()
				self.apic.connect(self.soma,1,0)
	...
	def d_lambda(self):
		...
		for dendType in self.dendTypeList:
			for sec in self. dendTypeList[dendType]:
				...
				sec.nseg = int((sec.L/(d_lambda*lambda_f)+0.9)/2)*2 + 1
				h.pop_section()
